...
module.exports.parse = function (id, content, directory, locale='en') {
  ...
  for (let i = 0; i < $md.length; ++i) {
    ...
    $md[i].innerHTML = marked($md[i].innerHTML, {renderer}).replace(/^<p>|<\/p>$/g, '');
  }
  ...
  for (let $th of body.querySelectorAll('thead')) {
    if (!$th.textContent.trim() && !$th.querySelector('.katex')) $th.remove();
  }
...
module.exports.parseSimple = function (text, locale='en') {
  const course = {bios: new Set(), gloss: new Set(), steps: [{}], title: ''};
...
const NOWRAP_QUERY = 'code, x-blank-input, x-blank, x-var, .katex, x-gloss, x-bio, span.step-target, span.pill, x-target, span.math';
